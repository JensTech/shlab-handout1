...
int main(int argc, char **argv) 
{
    ...
    /* Execute the shell's read/eval loop-infinite */
    while (1) {
...
//background process
    ...
    if(!builtin_cmd(argv))
    {
        ...
        sigprocmask(SIG_BLOCK, &mask, &prev);
         //pipes need to create as many pipes as cmds-1
    //create before you launch the child
       ...
       // int numcmds = parseargs(argv, cmds, stdin_redir, stdout_redir);
        ...
        //{
            ...
            if(pid ==0)
            {
                //I am the child and should launch process
                    //i/o redirects.... who cares about them... ONLY THE CHILD DOES. do them vefore the execv
                        //do that with the open (o read only, o write only,) input pipe, output pipe do 'dup'ping before execv
                        //Do i/o redirects working 1st then piping
                        //make sure you are cleaning up
            //step by step INCREMNTALLY PROGRAMMING
                        //pipe between multiple children
                setpgid(0,0);
...
{
    //check to see if argvector has commandin
    if(strcmp(argv[0],"quit") ==0)
        exit(0);
    else if (strcomp(argv[0], "jobs")){
      //  listjobs(jobs);
      printf("TEST WORKED\n");
        return 1;
    }
